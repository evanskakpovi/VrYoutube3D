 ...
 */
public class Floating2DVideos extends CardboardActivity implements CardboardView.StereoRenderer {
...
//          //System,out.println(dataCopy.get(i).myBitmap);
    ...
    overlayView.show3DToast("Let get started! Select a video to watch.");
...
//        if (pref.getBoolean(var.run1[2], true)) {
//          //  new var.voice().execute(getString(R.string.hello2d));
//            editor.putBoolean(var.run1[2], false);
//            editor.commit();
//        }

        mTracker.setScreenName("2D IMMERSIVE VIEW");
...
//                  //System,out.println("york");
    ...
    private void ActionToPerform() {
        ...
        if (floatingImages.get(0).isLookingAtObject(headView, PITCH_LIMIT, YAW_LIMIT)) {
            ...
            Intent id = new Intent(Floating2DVideos.this,
...
//                page = pages.size()-1;
            ...
            if (!vidBools) {
                String idForList="PLcQCsePRL4jcLU_mhIsejZ9qaPumkbgUv";
                ...
                new LoadPlayList().execute(url,"2");
                //System,out.println("2d");
                win= true; }
            else if (vidBools) {
                try {
                    String  url = searchUrl + and + part + and + type+and+ pageToken + pages.get(page) + and + max + and + term + URLEncoder.encode(searchQuery, "UTF-8") + and + MyAcessTokenData + and+key;
                    new LoadVideos().execute(url,"2");
            ...
            Intent id = new Intent(Floating2DVideos.this,
                    Floating2DSearch.class).addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY);
//          ids.putExtra(SelectActivity.vidId2, dataCopy.get(i).ids);
            finish();
            startActivity(id);
            win = true;
        ...
        for(int i=3;i<floatingImages.size();i++){
            if (floatingImages.get(i).isLookingAtObject(headView, PITCH_LIMIT, YAW_LIMIT)) {
              getURI(dataCopy.get(i-3).id, is360(dataCopy.get(i-3).getTitle(), dataCopy.get(i-3).getChannel()));
//            finish();
//            win = true;
            }
//        if (floatingImages.get(i).isLookingAtObject(headView, PITCH_LIMIT, YAW_LIMIT)) {
...
//                        idsInt.add(0);
//                        ids.add(ids);
                        stat.myData.add(new data(imageUrl, title, channeltitle,0,Floating2DVideos.this, id));
                        //System,out.println(stat.myData.get(i));
                    }
...
//                    getAccountNames();
        ...
        protected void onPostExecute(String result) {
          ...
          //  for (int i=0; i<stat.myDataAll.size(); i++)
            ...
            {
                page++;
                Intent id = new Intent(Floating2DVideos.this, Floating2DVideos.class).addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY);
...
//                        idsInt.add(1);
                        ...
                        stat.myData.add(new data(imageUrl, title, channeltitle,0,Floating2DVideos.this,idchan));
...
//                        ids.add(idchan);
//                        idsInt.add(1);
                        stat.myData.add(new data(imageUrl, title, channeltitle,1,Floating2DVideos.this,idchan));
                    } else {
                        continue;
                    }
...
//                    getAccountNames();
        ...
        protected void onPostExecute(String result) {
                ...
                stat.myDataAll.clear();
                stat.myDataAll.addAll(stat.myData);
//                //System,out.println(myDataAll.size());
//                for (int i=0; i<myDataAll.size(); i++)
//                    //System,out.println(myDataAll.get(i).myBitmap);
            ...
            YouTubeUriExtractor ytEx = new YouTubeUriExtractor(this) {
                ...
                public void onUrisAvailable(String videoId, String videoTitle, SparseArray<YtFile> ytFiles) {
                    if (ytFiles != null) {
                        ...
                        if (downloadUrl!=null){
//                            Intent intent = new Intent(Floating2DVideos.this, VideoPlayerActivity.class);
//                            intent.putExtra(MainStarterActivity.vidId, downloadUrl);
//                            startActivity(intent);
                           videoPlayer(downloadUrl);
                        } else {
                    ...
                    public void onUrisAvailable(String videoId, String videoTitle, SparseArray<YtFile> ytFiles) {
                        ...
                        if (ytFiles != null) {
                            ...
                            if (downloadUrl2 != null && !downloadUrl2.isEmpty()) {
                                ...
                                Intent intent = new Intent(Floating2DVideos.this, SimpleVrVideoActivity.class);
                                intent.putExtra(MainStarterActivity.vidId, downloadUrl);
//                                intent.putExtra(SimpleVrVideoActivity.TAG_VIDEO_URL, MainStarterActivity.strVideoDownloadPath);
    ...
    private void play360(String url){
        ...
        intent.putExtra(SimpleVrVideoActivity.TAG_VIDEO_URL, url );
        startActivity(intent);

        mTracker.send(new HitBuilders.EventBuilder()
                .setCategory("New 360")
                .setAction("played from 2D in VR")
                .setLabel(url)
                .setValue(1)
                .build());
    }
